import java.util.*;
public class Accesories {
    void access()
    {
        int void1;
        do {
            System.out.println("1.Chain\n2.Ring\n3.Watch\n4.Belt");
            Scanner os = new Scanner(System.in);
            Scanner x = new Scanner(System.in);
            int[] id = {1, 2, 3, 4};
            int idc = 10;
            int cdc = 30;
            int rdc = 10;
            int qty;
            String[] color = new String[5];
            int[] rate = new int[5];
            int ch1_1 = x.nextInt();
            switch (ch1_1) {
                case 1: {
                    color = new String[]{"Silver Ring Dollar Chain", "Gold Silver Chain", "Silver Name pendant Chain", "Silver Round Chain"};
                    rate = new int[]{189, 189, 199, 199};
                    break;

                }
                case 2: {
                    color = new String[]{"Plain Silver", "Plain Silver(White Stone", "Plain Gold", "Plain Gold(White Stone)"};
                    rate = new int[]{189, 199, 199, 209};
                    break;
                }
                case 3: {
                    color = new String[]{"Casio G-Shock", "Titan Marine(Automatic)", "Rolex(Day-Date)", "Armani Exchange(Black)"};
                    rate = new int[]{15000, 30000, 9790000, 19000};
                    break;
                }
                case 4: {
                    color = new String[]{"Black", "Brown", "Black/Brown(INVERTIBLE)"};
                    rate = new int[]{999, 999, 1999};
                    break;
                }
                default:
                    System.out.println("Invalid");
            }
            String fo = String.format("%s-" + "%10s-" + "%30s-", "ID", "Model", "Price");
            System.out.println(fo);
            System.out.println("-".repeat(idc + cdc + rdc));
            for (int i = 0; i < id.length; i++) {
                String fid = String.format("%-" + idc + "d", id[i]);
                String fmd = String.format("%-" + cdc + "s", color[i]);
                String fpr = String.format("%-" + rdc + "d", rate[i]);
                System.out.println(fid + fmd + fpr);
            }
            int ko;
            do {
                System.out.print("Enter the item you want : ");
                int item = os.nextInt();
                System.out.print("Enter quantity : ");
                qty = os.nextInt();
                for (int i = 0; i < 50; i++) {
                    if ((Fashion.bill_name[i]) == null) {
                        Fashion.bill_name[i] = color[item - 1];
                        Fashion.bill_item_cost[i] = rate[item - 1];
                        Fashion.bill_item_qty[i] = qty;
                        Fashion.bill_item_amt[i] = (Fashion.bill_item_cost[i]) * (Fashion.bill_item_qty[i]);
                        Fashion.total += Fashion.bill_item_cost[i] * Fashion.bill_item_qty[i];
                        break;
                    } else if ((Fashion.bill_name[i]).equals(color[item - 1])) {
                        if (!((Fashion.bill_item_cost[i]) == (rate[item - 1]))) {
                            for (int j = i + 1; j < 50; j++) {
                                if (Fashion.bill_name[j] == null) {
                                    Fashion.bill_name[j] = color[item - 1];
                                    Fashion.bill_item_cost[j] = rate[item - 1];
                                    Fashion.bill_item_qty[j] += qty;
                                    Fashion.bill_item_amt[j] = (Fashion.bill_item_cost[j]) * (Fashion.bill_item_qty[j]);
                                    break;
                                }
                            }
                        } else {

                            Fashion.bill_item_qty[i] += qty;
                        }
                        Fashion.total += Fashion.bill_item_cost[i] * Fashion.bill_item_qty[i];
                        break;
                    }
                }
                System.out.println("Enter 1 to purchase another model of same size or 0-check for more");
                ko=os.nextInt();
            }
            while(ko==1);
            System.out.println("Enter 1-to Go back to Accessories Menu 0-Main Menu");
            void1 = os.nextInt();
        }
        while(void1==1);
    }
}
